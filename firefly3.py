...
class Firefly3Client:
    ...
    def validate(self):
        ...
        if (
            len(self.name_format) == 0
            or BANK_ACCOUNT_NAME_PLACEHOLDER not in self.name_format
        ):
            self.logger.error(
                "Your firefly3 accounts name format must contain the bank account name placeholder: "
                + BANK_ACCOUNT_NAME_PLACEHOLDER
                + "."
            )
