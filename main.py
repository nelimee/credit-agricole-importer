...
def _create_default_config_file(logger, config):
    ...
    config.set(
        FIREFLY3_SECTION, ACCOUNTS_NAME_FORMAT_FIELD, ACCOUNTS_NAME_FORMAT_DEFAULT
    )
...
def _get_clis(logger, config):
    ...
    f3_cli.name_format = firefly3_section.get(
        ACCOUNTS_NAME_FORMAT_FIELD, ACCOUNTS_NAME_FORMAT_DEFAULT
    )
    ...
    f3_cli.auto_detect_transfers = (
        settings_section.get(AUTO_DETECT_TRANSFERS_FIELD, AUTO_DETECT_TRANSFERS_DEFAULT)
        == "True"
    )
    ...
    f3_cli.validate()
    return ca_cli, f3_cli, save_logs, max_logs, debug


def process_transactions(logger, ca_cli, f3_cli, f3_accounts_number):
    ...
    for account in ca_cli.get_accounts():
        ...
        for ca_transaction in ca_cli.get_transactions(account):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CONFIG_FILE):
        _create_default_config_file(logger, config)
    else:
        ca_cli, f3_cli, save_logs, max_logs, debug = _get_clis(logger, config)
        # Start main process
        logger.log(
        ...
        process_transactions(logger, ca_cli, f3_cli, f3_accounts_number)
